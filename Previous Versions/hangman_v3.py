...
class HangmanSolver:
    ...
    def _eig_letter_for_word(self, pattern, candidates, guessed):
        ...
        for l in letter_pool:
            ...
            expected_remaining -= 0.01 * self._affix_bonus(pattern, l)
    ...
    def _affix_bonus(self, pattern, letter):
        score = 0
        # suffixes
        if pattern.endswith("___") and letter in "ing":
            score += 5
        if pattern.endswith("__") and letter in "edr":
            score += 4
        if pattern.endswith("____") and letter in "tionment":
            score += 6
        ...
        for l in remaining_letters:
            ...
            for w in words_state:
                ...
                for i, ch in enumerate(w):
                    ...
                    # priors with safe smoothing
                    lp = self.letter_prior.get(l, 0.0)
                    pp = self.pos_prior[length][i].get(l, 0.01) if length in self.pos_prior and i in self.pos_prior[length] else 0.01
                    lb = self.left_bigram[left].get(l, 0.01) if left in self.left_bigram else 0.01
                    rb = self.right_bigram[l].get(right, 0.01) if l in self.right_bigram else 0.01

                    score += alpha*lp + beta*pp + gamma*(lb + rb)
                    # print(lp, pp, lb, rb, score)

                    # affix & orthographic heuristics
                    score += self._affix_bonus(w, l)
